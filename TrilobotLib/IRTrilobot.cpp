...
//int IRrecv::getMessage



// get code from IR remote
...
long IRrecv::decode(IRdataclass *IRdata) {
  ...
  IRdata->value    = data;
  IRdata->team     = (int)(data>>24);                // team number in top byte 
  IRdata->player   = (int)((data>>16) & 0x00FF);     // player number in 3rd byte
  IRdata->message  = (int) (data & 0x0000FFFF);      // message in 2 lower bytes
  IRdata->code_type = NEC;
  return DECODED;
}
